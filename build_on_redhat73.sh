#!/bin/bash
# 此编译脚本使得lcc-4.2在redhat-7.3系统上编译成功
...
copy_file_and_create_link() {
	...
	#build_fatal_error "create $BUILDDIR/include failed!"
	cp -p -R include/$ARCH/linux/* $BUILDDIR/include/
	#build_fatal_error "copy platform specific include file failed!"
	
	# 如果你正在linux上安装lcc,你必须在build目录中创建一个名叫gcc的链接文件
	# 来指向gcc的库目录(因为lcc使用gcc的C预处理和大多数gcc的头文件)
	ln -sv /usr/lib/gcc-lib/i386-redhat-linux/2.96 $BUILDDIR/gcc
	#build_fatal_error "create link file \'gcc\' failed!"
}
